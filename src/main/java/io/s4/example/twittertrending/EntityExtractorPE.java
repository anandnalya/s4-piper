...
public class EntityExtractorPE extends ProcessingElement {
    
    private final Stream<EntityEvent> hashtagSeenStream;
    private final Stream<EntityEvent> userMentionStream;
    private final Stream<EntityEvent> wordFrequecyStream;

    public EntityExtractorPE(App app, Stream<EntityEvent> hashtagSeenStream,
	    Stream<EntityEvent> userMentionStream,
	    Stream<EntityEvent> wordFrequecyStream) {
	
	super(app);
	this.hashtagSeenStream = hashtagSeenStream;
	this.userMentionStream = userMentionStream;
	this.wordFrequecyStream = wordFrequecyStream;
    }
	...
	extractWords(text); // word frequency stream

	extractEntity(text, '#', hashtagSeenStream); // #tag stream
	extractEntity(text, '@', userMentionStream); // @mention stream
	...
	for (String word : text.split(" ")) {
		...
		if (!stopWords.contains(word)) {  // ignore stop words
		    EntityEvent entityEvent = new EntityEvent(word.toLowerCase(),
			    1);
		    wordFrequecyStream.put(entityEvent);
		}
	    }
	}
    }

    private void extractEntity(String text, char c, Stream<EntityEvent> stream) {
	int index;
	int prevIndex = 0;
	int textLength = text.length();
	while ((index = text.indexOf(c, prevIndex)) != -1) {
	    prevIndex = index + 1;
	    ...
	    EntityEvent entityEvent = new EntityEvent(sb.toString().toLowerCase(),
		    1);
	    stream.put(entityEvent);
	}
    }
    ...
    private static List<String> stopWords = Arrays.asList(new String[] { "a",
